...
<div class="row">
  <div class="small-12 columns">
    ...
    <div class="panel callout">
      <p><strong>Covid-19 update:</strong> Our <strong>events and programs are only online</strong> until further notice. We’ll be using Zoom, <a href="https://www.facebook.com/SpurlockMuseum/" class="external">Facebook<span>(external link)</span></a> video, and other platforms to connect with you safely. See individual event listings below for specific details.</p>
    </div>
  </div>
</div>
                    ...
                    foreach($records as $record){
                            ...
                            if ( ($checkm==$month) && ($checky==$year) ) {
                              ...
                              if ($type1 == "Exhibit") {
                                ...
                                if (date('d', strtotime($dateStart)) == date('d', $date) || date('d', strtotime($dateEnd)) == date('d', $date)) {
                                $show_today = true;
                                }
                              # Else (if the event is not an exhibit)...
                                ...
                                if (strlen($dateEnd) != 0) {
                                  ...
                                  if (date('d', $date) >= date('d', strtotime($dateStart)) && date('d', $date) <= date('d', strtotime($dateEnd))) {
                                    $show_today = true;
                                  }
                      ...
                      echo "</div>
                          ...
                          if(!empty($timeStart)){
                            if (!empty($dateEnd) && ($dateStart != $dateEnd)) {
                              ...
                              <span class='visuallyhidden'>Event Time: </span>". $timeStart . " " . $zoneStartDisplay;
                            }

<?php 
//Events Index Page
  require_once(dirname(__FILE__).'/../db-support/Recalled-2020-06-03/check_db.php');
    ...
    <div id="container">
          ...
          <div class="row">
            ...
            <div class="small-12 columns">
              <div id="site-accordian" class="hide-for-medium-up tabpanel" role="tablist"> 
                ...
                Events<span aria-hidden="true" class="icon chevron right"></span><span class="visuallyhidden"> Submenu</span></div> 
      ...
      <!-- Main Content Begin-->
          		...
          		<div data-equalizer-mq="medium-up">
            		<h2 id="today-heading">Today's Events</h2>
								<div class="small-12 columns" data-equalizer="date-and-caption" data-equalizer-mq="medium-up">
      							...
      							<?php // following is formatted to preserve html indentation while php code has it's own indentation
		                    ...
		                    // add criterion to find requests
		                    ...
		                    if($preview == 0) { // if preview is false, only show published events
		                    ...
		                    // add find requests and sorting rules to data then find the records
                    		...
                    		foreach($records as $record) {
				                  ...
				                  $show_today = false; // false by default
				                  ...
				                  if ( ($checkm==$month) && ($checky==$year) ) { // If the event occurs in the current month and year
				                    if ($type1 == "Exhibit") { // If the event is an exhibit
				                      if (date('d', strtotime($dateStart)) == date('d', $date) || date('d', strtotime($dateEnd)) == date('d', $date)) { // If the current date is the same as the start date of the exhibit, or current date is the same as the end date of the exhibit
				                      	$show_today = true; // display the event
				                      } 
				                    } else { // else (the event is not an exhibit)
				                      if (strlen($dateEnd) != 0) { // If the event has an end date
				                        if (date('d', $date) >= date('d', strtotime($dateStart)) && date('d', $date) <= date('d', strtotime($dateEnd))) { // If the event start date has passed or the event starts today, and the event end date is today or a future day (i.e., the event is ongoing)
				                          $show_today = true; // show the event
				                        }
				                      } else { // Else (if the event start date is the same as the current date).
				                        if (date('d', $date) == date('d', strtotime($dateStart))) {
				                          $show_today = true; // show the event
				                        }
				                      }
				                    }
                    ...
                    echo "<li class=\"filter_option ";
			                        ...
			                        if ($type1 == "Special Event") {
					                    ...
					                    if (strpos($type2,'Family-Friendly') !== false) echo " Family-Friendly";
			                        if (strpos($type2, 'Exhibit-Related') !== false) echo " Exhibit-Related";
			                        if (strpos($type2, 'InHerCloset') !== false) echo " InHerCloset";
				            echo "\">	
                  		<div class='card'>";
						                  $month_char=$checkm;
						                  switch ($checkm) {
          							...
          							echo "<div data-equalizer-watch='date-and-caption' class='card'>
                        	...
                        	<div class="card-content image-caption">
                          	<div class="card-maininfo"> 
                        			<span class="title"><a href="event.php?ID='.$eventID.'">'.$eventTitle .'</a></span>';
                          	...
                          	echo '</div>
	                        	<div class="text-right">';
	                          	echo "<a class='export-button' style='' title='Save event to iCalendar, Outlook, etc' href='/events/export-event.php?ID='".$eventID.'"><span class="icon calendar-plus" aria-hidden="true" title="Save event to iCalendar, Outlook, etc"></span><span class="visuallyhidden">Save event to iCalendar, Outlook, etc</span></a>
	                          </div>
	                        </div>
	                      </div>'; //end card content end watchmen

	                      echo '<div >
	                      	<ul class="tag-band" data-equalizer-watch="tags">';
		                        	if(!empty($type1)){ 
				        ...
				        echo "<div class='callout panel'>There are no events scheduled for today.</div>" ;
				      	...
				      	echo '<div class="row">
				        	...
				        	<div class="small-12 medium-6 columns">
						      	<a class="button expand onepointfive" href="#upcoming-event-filter">Upcoming Events</a>
						      </div>
						    </div></div>
						</div>
	        </div>
			        ...
			        <div class="row">
			            ...
			            <div class="visuallyhidden">Filter by Event Type:</div>
				            <ul class="filter-nav" aria-controls="upcoming-events-cards">
				              <li class="active"><a href="#all" class="filter">All<span class="visuallyhidden"> Events</span></a></li>
					            ...
					            <li><a href="#Special-Event" class="filter">Special Events<span class="visuallyhidden"> Events</span></a></li>
				      ...
				      <div id="upcoming-events-cards">
            			...
            			<div class="small-12 medium-5 large-4 columns">
              			<a id="export-button" class="button onepointfive expand tiny" target="_blank" title="Save events to iCalendar, Outlook, etc" href="/events/export-event.php?ID=upcoming"><span class="icon calendar-plus prepend" aria-hidden="true"></span><span class="visuallyhidden">Add Event to Calendar</span>Add <span class="showing_all">All Upcoming</span> <span class="header_tag">Events</span> to Calendar</a> 
            			</div>
          			</div>
          			...
          			<div class="row">
				          <div class="small-12 columns">
				          	<div class="hidden panel callout" id="empty_category"><p>There are <strong>no upcoming events in this category</strong>.</p>
        							<div id="rental_button">
        								<p>Interested in renting a Museum space for your upcoming event? <a href="/contact/#eventscoordinator">Contact us</a> today!</p>
          							<a class="button onepointfive" href="/visit/facilities/rentals/">Rentals</a>
          						</div>
          					</div>

          					<div class="row" data-equalizer-mq="medium-up">
          						<div class="small-12 columns" data-equalizer="date-and-caption" data-equalizer-mq="medium-up">
  											<ul id="card-list" class="small-block-grid-1 medium-block-grid-2 large-block-grid-3" data-equalizer="tags" data-equalizer-mq="medium-up">
	                        ...
	                        echo "\"><div class='card'>";
	                              ...
	                              # If the event has a photo in the database, display it.
	                     			echo '<a href="event.php?ID='.$eventID.'"><img src="/img/'.$photo.'" alt="'.$photoAlt.' '.date('n/j/Y',strtotime($dateStart)).'"></a>';
	                              # Else display a placeholder image based on the event's type.
	                              } else {
                            echo"<a href='event.php?ID=".$eventID."'>
                            	<div data-equalizer-mq='medium-up' class='crop wide-crop placeholder ".$type1."' >
	                                ...
	                                if ($type1 == "Special Event") {
	                                  ...
	                                  echo "event-".$type1;
	                                	...
	                                	echo "'><span class=\"visuallyhidden\">"." ". date('n/j/Y',strtotime($dateStart))."</span>
	                                </span>
                              	</div>
                            	</div>
                          	</a>";   
                          	...
                          	echo "<div data-equalizer-watch='date-and-caption'>
                                ...
                                if(!empty($timeStart)) {
                                  ...
                                  // Else (the event has an end date that is the same as it's start date) display the start time of the event.
                                		...
                                		echo "<span><span aria-hidden='true' class='icon time prepend' title='Event Time'></span><span class='visuallyhidden'>Event Time: </span>".$timeStart;
                                  }
        ...
        */
        		...
        		<div class="row">
        			<div class="small-12 medium-10 medium-offset-1 columns">
      					<div class="callout panel text-center">Sorry, the events database is currently under maintenence. Please try again later.</div>
        			</div>
        		</div>
        	</div>
        </section>';
      ...
      $('.filter_option:gt('+ pages +')').hide().last().after(
        ...
        i++;
          ...
          // add ?+parameter after the url
         	location.href += "?" + i;
        } else { // else just replace the existing parameter with the new one
        ...
        if (hash_para != 'all'){
          ...
          $('.filter_option'+hashName + ':gt('+ pages +')').hide().last().after(
	        ...
	        if(window.location.hash.indexOf("?") == -1){
          	...
          	// if there is no parameter already attached to URL
       			if (location.href.indexOf("?") === -1) {
             	location.href += "?" + i; // add ?+parameter after the url
           	} else { // else just replace the existing parameter with the new one
		          var loc = location.href;
		          location.href = loc.split('?')[0] + "?" + i ;
	          }
        	...
        	$('.header_tag').text(eval('text["'+hash_para+'"]'));
         	...
         	$(".filter_option").each(function(){$(this).show();});
